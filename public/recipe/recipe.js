async function renderRecipe() {
  ...
  if (result.message) {
    $(".flex-container").append(
      `<h1 class="page-title" >${result.message}</h1>`
    );
  }
  $(".flex-container").append(
    `<div>
  ...
  $("#img-container").css(
    "background-image",
    `url(./../global/images/${result.recipe.img}.jpg)`
  );
  $("#ingredients").append(
    `<h3 class="ingredients-headder">Ingredients List</h3><table class="table table-hover"><tbody></tbody></table>`
  );
...
function renderComment(comment, user_id) {
  let response = `<div class="comment">
  ...
  response += `<p class="comment-text">${comment.comment}</p>
    </div>`;
  return response;
}
