...
function generateRecipe(recipe, container) {
  ...
  if (
    ...
    container === "fridge-recipes-container"
  ) {
    imageCol = "col-md-3";
  } else {
    imageCol = "col-12";
  }
  return `<div class="${imageCol} account-recipe-item">
...
async function updateModal(recipe_name) {
  ...
  $("#recipe_description").val(
    result.recipe.description.replace(/^\s*\s/gm, "")
  );
  result.ingredients.map((ingredient) => {
    if ($("#ingredientsArray").children().length !== 0) {
      numberOfNextContainer =
        Number(
          $("#ingredientsArray").children().last().attr("id").split("-")[1]
        ) + 1;
    } else {
...
async function renderMyRecipes(container, filter = "") {
  ...
  if (user_id || container === "recipes-container") {
    ...
    } else if (container !== "recipes-container" && user_id) {
    ...
    if (container === "recipes-container") {
      $(".recipes")
        .prepend(renderSortingPaging())
        .append(renderSortingPaging());
      $(
...
function addIngredientField() {
  ...
  renderIngredients(
    `input-${numberOfNextContainer}`,
    `select-${numberOfNextContainer}`
  );
  $(`#select-${numberOfNextContainer}`).on("change", function () {
    renderMesures(
      `input-${numberOfNextContainer}`,
      $(`#select-${numberOfNextContainer} option:selected`).val()
    );
  });
}
...
async function addNewIngredient(e) {
  ...
  ingredientForm.append(
    "measure_id",
    $("#ingredientForm option:selected").val()
  );
...
async function submitForm(e) {
  ...
  if ($("#modalHeadder").text() === "Add recipe") {
    ...
    let nameCheck = await recipeNameCheck(
      formData.get("recipe_name"),
...
async function addFavorite(recipe_id, user_id, container) {
  ...
  if (result.message === "Like added.") {
    ...
    $(`#${recipe_id}`).text(
      "Sorry, try to add the recipe to your favorites later."
    );
  }
}
...
async function deleteFavorite(recipe_id, user_id, container) {
  ...
  if (result.message === "Like deleted.") {
    ...
    $(`#${recipe_id}`).removeAttr("hidden");
    $(`#${recipe_id}`).text(
      "Sorry, try to delete the recipe from your favorites later."
    );
  }
}
...
async function checkFavorite(recipe_id, container) {
  ...
  if (result && result.includes) {
    $(`#heart-icon-${container}-${recipe_id}`).attr(
      "src",
      "./../global/icons/LikedHeart.png"
    );
  }
}
